...
if [ -z "$1" ]; then
  ...
  if [ $1 == "macosx64" ]; then
    OUT_BINARY_PATH="${DIR}/build/native/Release"
    ...
    cp -rf "$SOURCE_PATH"/webviewpro-tests/src/main/java/tests "$DISTRIB_PATH"

    # Everything else is contained in the app bundle.
    cp -rf "$OUT_BINARY_PATH"/ceffx_app.app ""$DISTRIB_BIN_PATH
  else
    ...
    cp -f "$SOURCE_PATH"/webviewpro-core/target/webviewpro-core-*.jar "$DISTRIB_BIN_PATH/webviewpro-core.jar"

    # Copy test program source and JAR file to the bin directory.
    cp -rf "$SOURCE_PATH"/webviewpro-tests/src/main/java/tests "$DISTRIB_BIN_PATH"
    cp -f "$SOURCE_PATH"/webviewpro-tests/target/webviewpro-tests-*.jar "$DISTRIB_BIN_PATH/webviewpro-tests.jar"
    ...
    cp -f "$OUT_BINARY_PATH"/libceffx.so "$DISTRIB_LIB_PATH"
    cp -f "$OUT_BINARY_PATH"/ceffx_helper "$DISTRIB_LIB_PATH"
