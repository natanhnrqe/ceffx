...
the "required" section must be redistributed with all applications using CEFFX.
Components listed under the "optional" section may be excluded if the related
features will not be used.

Applications using CEFFX on OS X must follow a specific app bundle structure.
Replace "ceffx_app" in the below example with your application name.
    ...
    Frameworks/
      ...
      ceffx Helper.app/
        Contents/
          Info.plist
          MacOS/
            ceffx Helper <= helper executable
          Pkginfo
      Info.plist
    Java/
      *.jar <= Required JAR files
      libceffx.dylib <= CEF JNI library
    MacOS/
...
contains CEF binaries and resources. Binaries (libceffx.dylib, ceffx Helper,
etc) are linked to the "Chromium Embedded Framework" library using
install_name_tool and a path relative to @executable_path.

The "ceffx Helper" app is used for executing separate processes (renderer,
