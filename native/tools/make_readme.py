...
def create_readme():
  ...
  # format the file
  data = header_data + '\n\n' + mode_data + '\n\n' + redistrib_data + '\n\n' + footer_data
  data = data.replace('$CEFFX_URL$', ceffx_url)
  data = data.replace('$CEFFX_REV$', ceffx_commit_hash)
  data = data.replace('$CEFFX_VER$', ceffx_ver)
...
disc = """
This utility builds the CEFFX README.txt for the distribution.
"""
...
# CEFFX root directory
ceffx_dir = os.path.abspath(os.path.join(script_dir, os.pardir))

# Read and parse the CEF version file.
args = {}
read_readme_file(os.path.join(ceffx_dir, 'ceffx_build', 'README.txt'), args)

# retrieve url and revision information for CEF
if not git.is_checkout(ceffx_dir):
  raise Exception('Not a valid checkout: %s' % (ceffx_dir))

ceffx_commit_number = git.get_commit_number(ceffx_dir)
ceffx_commit_hash = git.get_hash(ceffx_dir)
ceffx_url = git.get_url(ceffx_dir)
ceffx_ver = '%s.%s.%s.%s+g%s' % (args['CEF_MAJOR'], args['CEF_MINOR'],
                                args['CEF_PATCH'], ceffx_commit_number,
                                ceffx_commit_hash[:7])
