...
if [ -z "$1" ]; then
  ...
  if [ -z "$2" ]; then
    ...
    OUT_PATH="${DIR}/java/webviewpro-core/target/classes"
    OUT_PATH="$OUT_PATH:${DIR}/java/webviewpro-tests/target/classes"
    echo $OUT_PATH

    LIB_PATH="${DIR}/build/native/$2"
    if [ ! -d "$LIB_PATH" ]; then
    ...
    # Necessary for ceffx_helper to find libcef.so.
