...
CefMessageRouterConfig GetJNIMessageRouterConfig(JNIEnv* env, jobject jConfig) {
  ...
  ScopedJNIClass cls(env,
                     "com/techsenger/ceffx/core/browser/CefMessageRouter$CefMessageRouterConfig");
  if (cls == nullptr)
    return config;
...
jobject NewJNIErrorCode(JNIEnv* env, cef_errorcode_t errorCode) {
  ...
  switch (errorCode) {
    default:
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_NONE,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_IO_PENDING,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_FAILED,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_ABORTED,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
               ERR_INVALID_ARGUMENT, jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
               ERR_INVALID_HANDLE, jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
               ERR_FILE_NOT_FOUND, jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_TIMED_OUT,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
               ERR_FILE_TOO_BIG, jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_UNEXPECTED,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
      ...
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_FILE_EXISTS,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
      ...
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_MSG_TOO_BIG,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
      ...
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_WS_UPGRADE,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
      ...
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_CERT_END,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_INVALID_URL,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
      ...
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_UNSAFE_PORT,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
      ...
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_CACHE_MISS,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
      ...
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode", ERR_CACHE_RACE,
               jerrorCode);
      JNI_CASE(env, "com/techsenger/ceffx/core/handler/CefLoadHandler$ErrorCode",
...
CefSize GetJNISize(JNIEnv* env, jobject obj) {
  ...
  ScopedJNIClass cls(env, "javafx/geometry/Dimension2D");
  if (!cls)
    return size;

  jmethodID getWidth = env->GetMethodID(cls, "getWidth", "()D");
  jmethodID getHeight = env->GetMethodID(cls, "getHeight", "()D");

  if (!getWidth || !getHeight)
    return size;

  jdouble width = env->CallDoubleMethod(obj, getWidth);
  jdouble height = env->CallDoubleMethod(obj, getHeight);

  size.Set(static_cast<int>(std::round(width)),
...
CefRect GetJNIRect(JNIEnv* env, jobject obj) {
  ...
  jmethodID midMinX = env->GetMethodID(cls, "getMinX", "()D");
  jmethodID midMinY = env->GetMethodID(cls, "getMinY", "()D");
  jmethodID midW    = env->GetMethodID(cls, "getWidth", "()D");
  jmethodID midH    = env->GetMethodID(cls, "getHeight", "()D");

  if (!midMinX || !midMinY || !midW || !midH)
    return rect;

  jdouble x = env->CallDoubleMethod(obj, midMinX);
  jdouble y = env->CallDoubleMethod(obj, midMinY);
  jdouble w = env->CallDoubleMethod(obj, midW);
  jdouble h = env->CallDoubleMethod(obj, midH);
...
bool GetJNIPoint2D(JNIEnv* env, jobject obj, int* x, int* y) {
  if (!env || !obj || !x || !y)
    return false;
  ...
  jmethodID getX = env->GetMethodID(cls, "getX", "()D");
  jmethodID getY = env->GetMethodID(cls, "getY", "()D");

  if (!getX || !getY)
    return false;

  jdouble dx = env->CallDoubleMethod(obj, getX);
  jdouble dy = env->CallDoubleMethod(obj, getY);
