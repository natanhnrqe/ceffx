...
// ...except if there's a change in the classloader to use by CEFFX, in which
...
jclass FindClass(JNIEnv* env, const char* class_name) {
  ...
  // never released! However, for the classes that are requested by CEFFX via
...
jobject NewJNIBoolRef(JNIEnv* env, bool initValue) {
  ScopedJNIObjectLocal jboolRef(env, "com/techsenger/ceffx/core/misc/BoolRef");
  if (!jboolRef)
    return nullptr;
...
jobject NewJNIIntRef(JNIEnv* env, int initValue) {
  ScopedJNIObjectLocal jintRef(env, "com/techsenger/ceffx/core/misc/IntRef");
  if (!jintRef)
    return nullptr;
...
jobject NewJNILongRef(JNIEnv* env, int64_t initValue) {
  ScopedJNIObjectLocal jlongRef(env, "com/techsenger/ceffx/core/misc/LongRef");
  if (!jlongRef)
    return nullptr;
...
jobject NewJNIStringRef(JNIEnv* env, const CefString& initValue) {
  ScopedJNIObjectLocal jstringRef(env, "com/techsenger/ceffx/core/misc/StringRef");
  if (!jstringRef)
    return nullptr;
...
jobject NewJNICookie(JNIEnv* env, const CefCookie& cookie) {
  ...
  return NewJNIObject(env, "com/techsenger/ceffx/core/network/CefCookie",
...
jobject NewJNITransitionType(JNIEnv* env,
  ...
  switch (transitionType & TT_SOURCE_MASK) {
    default:
      JNI_CASE(env, "com/techsenger/ceffx/core/network/CefRequest$TransitionType", TT_LINK,
               result);
      JNI_CASE(env, "com/techsenger/ceffx/core/network/CefRequest$TransitionType", TT_EXPLICIT,
               result);
      JNI_CASE(env, "com/techsenger/ceffx/core/network/CefRequest$TransitionType",
               TT_AUTO_SUBFRAME, result);
      JNI_CASE(env, "com/techsenger/ceffx/core/network/CefRequest$TransitionType",
               TT_MANUAL_SUBFRAME, result);
      JNI_CASE(env, "com/techsenger/ceffx/core/network/CefRequest$TransitionType", TT_FORM_SUBMIT,
               result);
      JNI_CASE(env, "com/techsenger/ceffx/core/network/CefRequest$TransitionType", TT_RELOAD,
               result);
  }
...
jobject NewJNIURLRequestStatus(
  ...
  switch (status) {
    default:
      JNI_CASE(env, "com/techsenger/ceffx/core/network/CefURLRequest$Status", UR_UNKNOWN, result);
      JNI_CASE(env, "com/techsenger/ceffx/core/network/CefURLRequest$Status", UR_SUCCESS, result);
      JNI_CASE(env, "com/techsenger/ceffx/core/network/CefURLRequest$Status", UR_IO_PENDING,
               result);
      JNI_CASE(env, "com/techsenger/ceffx/core/network/CefURLRequest$Status", UR_CANCELED,
               result);
      JNI_CASE(env, "com/techsenger/ceffx/core/network/CefURLRequest$Status", UR_FAILED, result);
  }
  return result.Release();
}
...
ScopedJNIPostDataElement::ScopedJNIPostDataElement(
    ...
    : ScopedJNIObject<CefPostDataElement>(
          env,
          obj,
          "com/techsenger/ceffx/core/network/CefPostDataElement_N",
          "CefPostDataElement") {}
...
ScopedJNIResourceReadCallback::ScopedJNIResourceReadCallback(
    ...
    : ScopedJNIObject<CefResourceReadCallback>(
          env,
          obj,
          "com/techsenger/ceffx/core/callback/CefResourceReadCallback_N",
...
ScopedJNIResourceSkipCallback::ScopedJNIResourceSkipCallback(
    ...
    : ScopedJNIObject<CefResourceSkipCallback>(
          env,
          obj,
          "com/techsenger/ceffx/core/callback/CefResourceSkipCallback_N",
