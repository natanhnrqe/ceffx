...
void ClientApp::OnBeforeCommandLineProcessing(
  ...
  if (handle_ && process_type.empty()) {
    ...
    ScopedJNIObject<CefCommandLine> jcommandLine(
        env, command_line, "com/techsenger/ceffx/core/callback/CefCommandLine_N",
        "CefCommandLine");
    jcommandLine.SetTemporary();

    JNI_CALL_VOID_METHOD(
        env, handle_, "onBeforeCommandLineProcessing",
        "(Ljava/lang/String;Lcom/techsenger/ceffx/core/callback/CefCommandLine;)V",
...
void ClientApp::OnRegisterCustomSchemes(
  ...
  ScopedJNIObject<CefSchemeRegistrar, CefRawPtr<CefSchemeRegistrar>> jregistrar(
      env, registrar, "com/techsenger/ceffx/core/callback/CefSchemeRegistrar_N",
      "CefSchemeRegistrar");
  jregistrar.SetTemporary();

  JNI_CALL_VOID_METHOD(env, handle_, "onRegisterCustomSchemes",
                       "(Lcom/techsenger/ceffx/core/callback/CefSchemeRegistrar;)V",
                       jregistrar.get());
}
...
bool ClientApp::HandleTerminate() {
  ...
  ScopedJNIClass jcls(env, "com/techsenger/ceffx/core/CefApp");
  if (!jcls) {
    return false;
  }

  jmethodID methodId =
      env->GetStaticMethodID(jcls, "getInstance", "()Lcom/techsenger/ceffx/core/CefApp;");
  if (!methodId) {
    return false;
  }

  ScopedJNIObjectLocal ceffxApp(env,
                               env->CallStaticObjectMethod(jcls, methodId));
  if (!ceffxApp) {
    return false;
  }

  JNI_CALL_VOID_METHOD(env, ceffxApp, "handleBeforeTerminate", "()V");
  return true;
}
#endif  // defined(OS_MACOSX)
