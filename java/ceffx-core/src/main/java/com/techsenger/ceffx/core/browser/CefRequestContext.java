...
import com.techsenger.ceffx.core.handler.CefRequestContextHandler;
...
public abstract class CefRequestContext {
     ...
     * <p>
     * This method must be called on the browser process CEF main thread, otherwise it will always return
     * false. It is easiest to ensure the correct calling thread by using a callback method invoked
     * by the browser process CEF main thread, such as CefLifeSpanHandler.onAfterCreated(CefBrowser), to
     ...
     * be called on the browser process CEF main thread, otherwise it will always return null.
     */
     ...
     * process CEF main thread, otherwise it will always return false.
     */
