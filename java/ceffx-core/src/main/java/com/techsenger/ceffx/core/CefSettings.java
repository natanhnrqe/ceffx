...
public class CefSettings {
    ...
    /**
     * Set to true to enable multi-threaded message loop mode. When enabled,
     * CEF will create and manage its own message loop thread for processing
     * browser events (mouse, keyboard, network, timers, etc.).
     *
     * When enabled with OSR, CEF will handle all message processing internally
    ...
    /**
     * Set to true to enable external message pump mode. When enabled, CEF will not
     * create its own message loop thread. Instead, you must call doMessageLoopWork()
     * regularly (typically every 10-20ms) to process CEF events.
     *
     * This mode is useful when you need explicit control over when CEF processes
     * events, but requires careful integration with your application's event loop.
     *
     * Note: You should not enable both multi_threaded_message_loop and
