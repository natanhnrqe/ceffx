...
import com.techsenger.ceffx.core.browser.CefDevToolsClient.DevToolsException;
...
import com.techsenger.ceffx.core.handler.CefClientHandler;
import com.techsenger.ceffx.core.handler.CefDialogHandler.FileDialogMode;
...
abstract class CefBrowser_N extends CefNativeAdapter implements CefBrowser {
    ...
    private volatile Point2D inspectAt_ = null;
    ...
    protected CefBrowser_N(CefClient client, String url, CefRequestContext context,
            CefBrowser_N parent, Point2D inspectAt, CefBrowserSettings settings) {
        client_ = client;
        url_ = url;
    ...
    protected Point2D getInspectAt() {
        return inspectAt_;
    }
...
//        });
    ...
    @Override
    public synchronized void openDevTools(double x, double y) {
        Point2D inspectAt = null;
        if (x >= 0 && y >= 0) {
            inspectAt = new Point2D(x, y);
        }
    ...
    /**
     * Called async from the (native) main CEF main thread.
     */
    ...
    protected final void createDevTools(CefBrowser_N parent, CefClientHandler clientHandler,
            long windowHandle, boolean osr, boolean transparent, Point2D inspectAt) {
        if (getNativeRef("CefBrowser") == 0 && !isPending_) {
            try {
     ...
     * @param e The event to send.
     */
    protected final void sendMouseWheelEvent(ScrollEvent e) {
        try {
    ...
    protected final void dragTargetDragEnter(
            CefDragData dragData, Point2D pos, int modifiers, int allowedOps) {
        try {
     ...
     * This method is only used when window rendering is disabled.
     */
    protected final void dragTargetDragOver(Point2D pos, int modifiers, int allowedOps) {
        try {
     ...
     * This method is only used when window rendering is disabled.
     */
    protected final void dragTargetDrop(Point2D pos, int modifiers) {
        try {
     ...
     * This method is only used when window rendering is disabled.
     */
    protected final void dragSourceEndedAt(Point2D pos, int operation) {
        try {
    ...
    protected final void updateUI(BoundingBox contentRect, BoundingBox browserRect) {
        try {
    ...
    protected final void setParent(long windowHandle) {
        if (isClosing_ || isClosed_) return;

        try {
            N_SetParent(windowHandle);
        } catch (UnsatisfiedLinkError ule) {
            ule.printStackTrace();
        }
    }
    ...
    private final native boolean N_CreateBrowser(CefClientHandler clientHandler, long windowHandle,
            String url, boolean osr, boolean transparent, CefRequestContext context, CefBrowserSettings settings);
    ...
    private final native void N_SendMouseWheelEvent(ScrollEvent e);
    private final native void N_DragTargetDragEnter(
            CefDragData dragData, Point2D pos, int modifiers, int allowed_ops);
    private final native void N_DragTargetDragOver(Point2D pos, int modifiers, int allowed_ops);
    private final native void N_DragTargetDragLeave();
    private final native void N_DragTargetDrop(Point2D pos, int modifiers);
    private final native void N_DragSourceEndedAt(Point2D pos, int operation);
    private final native void N_DragSourceSystemDragEnded();
    private final native void N_UpdateUI(BoundingBox contentRect, BoundingBox browserRect);
    private final native void N_SetParent(long windowHandle);
