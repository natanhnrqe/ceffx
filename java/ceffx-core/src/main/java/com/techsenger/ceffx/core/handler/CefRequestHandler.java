...
public interface CefRequestHandler {
    ...
    /**
     * Called on the CEF main thread before browser navigation.
     *
    ...
    /**
     * Called on the CEF main thread before OnBeforeBrowse in certain limited cases
    ...
    /**
     * Called on the CEF main thread to handle requests for URLs with an invalid SSL certificate. If
    ...
    /**
     * Called on the browser process CEF main thread when the render process terminates unexpectedly.
     * @param browser The corresponding browser.
